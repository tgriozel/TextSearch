...
class StringPrefixSearcherSpec extends Specification {
  ...
  "The string prefix searcher" should {
    "Return correct results" in {
      searcher.getSomeWordsWithPrefix("p") mustEqual Set("pandora", "paypal", "pg&e", "phone")
      searcher.getSomeWordsWithPrefix("pr") mustEqual Set("prank", "press democrat", "print", "proactive")
      searcher.getSomeWordsWithPrefix("prog") mustEqual Set("progenex", "progeria", "progesterone", "programming")
    }

    "Return only the required amount of suggestions" in {
      searcher.getSomeWordsWithPrefix("p", count = 2) mustEqual Set("pandora", "paypal")
    }
    ...
    "Return the same results with a less complex structure, for a simple enough research" in {
      ...
      searcherTwo.getSomeWordsWithPrefix("p") mustEqual Set("pandora", "paypal", "pg&e", "phone")
    }
  }
}
